...
/* ──────────────────────────────────────────────────────────
   ...
   registration list. Light band; copy left, draw card right
   (mirrors the navy Registration section above for rhythm).
   ────────────────────────────────────────────────────────── */
...
export function LuckyDraw() {
  ...
  return (
    <section id="lucky-draw" className="bg-brand-paper border-y border-brand-line relative overflow-hidden">
      ...
      <Container className="relative py-20 md:py-28">
        <div className="grid lg:grid-cols-[1fr_400px] items-center gap-12 lg:gap-20">
          ...
          <div className="reveal order-2 lg:order-1 text-center lg:text-left">
            ...
            <h2 className="h-display mt-6 text-brand-ink" style={{ fontSize: "clamp(34px, 4.6vw, 60px)" }}>
              {l.title}
            </h2>
            <span aria-hidden className="block w-12 h-px bg-brand-red mt-6 mx-auto lg:mx-0" />
            <p className="mt-6 max-w-[60ch] mx-auto lg:mx-0 text-[16px] md:text-[18px] font-light leading-[1.75] text-brand-mute">
              {l.body}
            </p>

            <ul className="mt-8 grid sm:grid-cols-3 gap-4 list-none p-0 m-0">
              {l.proof.map((pf, i) => (
                <li key={i} className="flex items-center justify-center lg:justify-start gap-2.5 rounded-xl border border-brand-line bg-white px-4 py-3 text-[13px] font-medium text-brand-ink">
          ...
          <div className="reveal order-1 lg:order-2 mx-auto w-full max-w-[360px] float-soft">
            <div className="rounded-3xl bg-brand-ink text-white p-6 shadow-[0_30px_70px_-20px_rgba(6,10,30,.6)]">
              ...
              <div className="relative mt-5 h-[180px] overflow-hidden rounded-2xl bg-white/[0.04] ring-1 ring-white/10">
                {/* center highlight */}
                <div aria-hidden className="absolute inset-x-4 top-1/2 -translate-y-1/2 h-12 rounded-lg border border-brand-red/50 bg-brand-red/10 z-10" />
